...
// Update while loading (experimental)
#define REFRESH_INTERMEDIATE

// Automatically perform a SDL_Flip on update
#define NEED_FLIP
...
#ifdef REFRESH_INTERMEDIATE
	int intermediate_rectangles = 0;
#endif

#ifdef ALLOW_ZOOMING
	#include "../SDL_rotozoom-1.6/SDL_rotozoom.h"
#endif
...
#ifdef REFRESH_INTERMEDIATE
						intermediate_rectangles++;
						if (intermediate_rectangles >= 7) {
							intermediate_rectangles = 0;
							vncRefreshIntermediate();
						}
#endif
...
void zoomBlit(SDL_Surface *src, SDL_Rect *srcRec, SDL_Surface *dest, SDL_Rect *destRec, float zoom) {
	// No zoom:
	...
	// Copy source image to origin of temp
	SDL_Surface *temp = SDL_CreateRGBSurface(SDL_SWSURFACE, srcRec->w, srcRec->h, 32, lastVnc->rmask, lastVnc->gmask, lastVnc->bmask, 0);
	SDL_Rect tempRec;
	tempRec.x = 0;
	tempRec.y = 0;
	tempRec.w = srcRec->w;
	tempRec.h = srcRec->h;
	SDL_BlitSurface(src, srcRec, temp, &tempRec);
	
	// Manual offset:
	//@TODO: Make rotoZoom handle offset!
	// Scale temp
	if ((rotozoom_picture = zoomSurface(temp, zoom, zoom, smooth)) != NULL) {
		// Blit zoomed picture to dest
		SDL_BlitSurface(rotozoom_picture, NULL, dest, destRec);
		SDL_FreeSurface(rotozoom_picture);
	}
	
	/*
	if ((rotozoom_picture = zoomSurface(temp, zoom, zoom, destRec->x, destRec->y, smooth)) != NULL) {
		// Blit zoomed picture to dest
		//SDL_BlitSurface(rotozoom_picture, NULL, dest, destRec);
		SDL_FreeSurface(rotozoom_picture);
	}
...
void vncRefreshIntermediate() {
	if (!lastVnc) return;
	if (!lastVnc->lastTarget) return;
	
	vncBlitFramebufferAdvanced(lastVnc, lastVnc->lastTarget, NULL, lastVnc->lastOutX, lastVnc->lastOutY, lastVnc->lastOutScale, 0);
}
...
int vncBlitFramebufferAdvanced(tSDL_vnc *vnc, SDL_Surface *target, SDL_Rect *urec, int outx, int outy, float outScale, int fullRefresh) {
	...
	lastVnc = vnc;
	vnc->lastTarget = target;
	vnc->lastOutX = outx;
	vnc->lastOutY = outy;
	vnc->lastOutScale = outScale;
	...
	if ((fullRefresh > 0) || vnc->fbupdated) {
		...
		if (fullRefresh > 0) {
...
#ifdef ALLOW_ZOOMING
			zoomBlit(vnc->framebuffer, &srcrec, target, &dstrec, outScale);
#else
			...
			// Incremental update
			SDL_Rect dstrec;
			dstrec.x = vnc->updatedRect.x * outScale + outx;
			dstrec.y = vnc->updatedRect.y * outScale + outy;
			dstrec.w = vnc->updatedRect.w * outScale;
			dstrec.h = vnc->updatedRect.h * outScale;
#ifdef ALLOW_ZOOMING
			zoomBlit(vnc->framebuffer, &vnc->updatedRect, target, &dstrec, outScale);
#else
