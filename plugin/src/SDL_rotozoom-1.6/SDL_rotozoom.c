...
//int zoomSurfaceY (SDL_Surface * src, SDL_Surface * dst, int destX, int destY)
{
  ...
  dp = (Uint8 *) dst->pixels;
  //dp = (Uint8 *) dst->pixels + (dst->w * destY * dst->pitch) + destX;
...
rotozoomSurface (SDL_Surface * src, double angle, double zoom, int smooth)
{
  ...
  if (fabs (angle) > VALUE_LIMIT)
      ...
      else
	{
	  ...
	  //zoomSurfaceY (rz_src, rz_dst, 0, 0);
	  SDL_SetColorKey (rz_dst, SDL_SRCCOLORKEY | SDL_RLEACCEL,
			   rz_src->format->colorkey);
	}
      /* Unlock source surface */
...
//SDL_Surface *zoomSurface (SDL_Surface * src, double zoomx, double zoomy, int destX, int destY, int smooth)
{
  SDL_Surface *rz_src;
  SDL_Surface *rz_dst;
  ...
  if (is32bit)
      ...
      for (i = 0; i < rz_src->format->palette->ncolors; i++)
	{
      ...
      /* Call the 8bit transformation routine to do the zooming */
      zoomSurfaceY (rz_src, rz_dst);
      //zoomSurfaceY (rz_src, rz_dst, destX, destY);
      SDL_SetColorKey (rz_dst, SDL_SRCCOLORKEY | SDL_RLEACCEL,
		       rz_src->format->colorkey);
    }
  /* Unlock source surface */
